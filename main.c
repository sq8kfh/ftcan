...
{
	if (command[0] == 'S') {	//Setup Bitrate
		//uint8_t s;
		//if (sscanf(command,"S%1hhu",&s) != 1 || command[2] != '\r')
		//goto error;
	}
	...
	else if (command[0] == 'T') {	//TX Frame Format EFF Tiiiiiiiildddddddd...
		if (slcan_command_T(command))
			FT220X_write('\r');
		else
			goto error;
	...
	else if (command[0] == 'U') {	//UART Speed Setup Ux (0 <= x <= 6)
		if (command[2] != '\r')
			goto error;
		FT220X_write('\r');
	}
